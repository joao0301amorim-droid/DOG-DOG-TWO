...
export const GoalsView: React.FC<GoalsViewProps> = ({
  ...
  const handleToggleGoalStatus = (goal: Goal) => {
    ...
    onSaveGoal({ ...goal, status: nextStatus });
  };
  ...
  const handleChangePriority = (goal: Goal, newPriority: GoalPriority) => {
    onSaveGoal({ ...goal, priority: newPriority });
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div
        ...
        className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-5 sm:p-6 rounded-2xl bg-[#111114] border border-[#1e293b] shadow-xl"
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2 text-[10px] uppercase font-mono font-bold tracking-widest text-indigo-400">
              ...
              <span>•</span>
              <span className="text-emerald-400">INTEGRAÇÃO FINANCEIRA TOTAL</span>
            </div>
            <h2 className="text-2xl sm:text-3xl font-black text-white tracking-tight flex items-center gap-3">
              <span>{monthSummary.monthName}</span>
            </h2>
            <p className="text-xs text-slate-400 mt-0.5">
              Defina a Meta Principal (🥇) e até 2 Metas Secundárias (🥈, 🥉) com ritmo ideal por dia e score de consistência.
            </p>
          </div>
        </div>

        {/* Month Selector Controls & Add Goal CTA */}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div
          ...
          className="p-5 rounded-2xl bg-[#0a1813] border border-emerald-500/40 flex flex-col justify-between shadow-sm relative overflow-hidden"
        >
          <div className="flex items-center justify-between">
            <span className="text-[10px] uppercase font-mono font-bold tracking-widest text-emerald-300">
              📈 Resultado Líquido
            </span>
            <span className="text-[10px] font-mono font-bold text-white bg-emerald-600 px-2 py-0.5 rounded">
              LIVRE
            </span>
          </div>
          <div className="mt-3">
            ...
            <p className="text-[11px] text-emerald-400/80 font-mono mt-1">
              Ganhos - Gastos = Líquido
            </p>
          </div>
        </div>

        {/* ⏱️ Tempo & Dias Restantes */}
        <div
          id="month-time-card"
        ...
        >
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs font-mono text-indigo-400 bg-indigo-500/10 px-2 py-0.5 rounded border border-indigo-500/20">
              {monthSummary.daysInMonth - monthSummary.currentDay} dias restam
            </span>
          </div>
          <div className="mt-3">
            <div className="flex items-baseline gap-1.5">
              <span className="text-2xl sm:text-3xl font-extrabold font-mono text-white tabular-nums">
                Dia {monthSummary.currentDay}
              </span>
              <span className="text-xs text-slate-500 font-mono">/ {monthSummary.daysInMonth}</span>
            </div>
      ...
      {primaryGoal ? (
        ...
        <div className="p-8 rounded-3xl bg-[#111114] border border-[#1e293b] border-dashed text-center">
          <Target className="w-10 h-10 text-slate-500 mx-auto mb-3" />
          <h3 className="text-lg font-bold text-white">Nenhuma Meta Principal cadastrada para este mês</h3>
          <p className="text-xs text-slate-400 max-w-md mx-auto mt-1 mb-4">
            Defina seu objetivo número 1 (ex.: Guardar R$ 850) para começar a acompanhar o ritmo diário e o score de sustentabilidade.
          </p>
          <button
            onClick={() => {
              setEditingGoal(null);
            ...
            className="px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-xs uppercase"
          >
                  ...
                  <div>
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <div className="flex items-center gap-1">
                        <button
                          onClick={() => handleChangePriority(goal, 'PRINCIPAL')}
                          className="text-[10px] font-mono text-amber-400 hover:text-amber-300 px-2 py-1 rounded bg-amber-500/10 border border-amber-500/20"
                          title="Promover a Meta Principal"
                        >
                          Tornar Principal
                        </button>
                        <button
